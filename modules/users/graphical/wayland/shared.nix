...
in {
  options.jd.graphical.wayland = {
    ...
    statusbar = {
      ...
      pkg = mkOption {
        type = types.package;
        default = if (isDwl || isRiver) then
          pkgs.waybar-master.override {
            swaySupport = false;
  ...
  config = mkIf cfg.enable (mkMerge [
      ...
      programs.foot = {
        ...
        in {
          main = {
            pad = "2x2 center";
            font =
              "Berkeley Mono Variable:size=20,Noto Color Emoji:style=Regular:size=20";
          };
          cursor = { color = "282a36 f8f8f2"; };
          colors =
            if (cfg.foot.theme == "tokyo-night") then tokyoNight else dracula;
        };
      };
          ...
          (mkMerge [
            {
              ...
              modules = {
                ...
                backlight = {
                  ...
                  on-click =
                    "${toggleservice}/bin/toggleservice autobrightness.service";
                } // (optionalAttrs cfg.screen.gamma.enable {
                  on-click-right =
                    "${toggleservice}/bin/toggleservice wlsunset.service";
                });
                pulseaudio = {
                  ...
                  on-scroll-up =
                    "${pkgs.scripts.soundTools}/bin/stools vol up 1";
                  on-scroll-down =
                ...
                network = {
                  ...
                  tooltip = true;
                  tooltip-format =
                    "{ifname} via {gwaddr}: {bandwidthUpBytes} 󰳘 {bandwidthDownBytes} 󰱦";
                };
                idle_inhibitor = {
                  format = "{icon}";
                  format-icons = {
            ...
            (mkIf (cfg.type == "river") {
              modules-left =
                [ "river/tags" "river/mode" "river/layout" "river/window" ];
            })
          ])
          (mkIf (!isLaptop) (mkMerge [
            (mkIf isDwl {
              output = "HDMI-A-1";
    ...
    (mkIf cfg.screen.gamma.enable {
      ...
      systemd.user.services.wlsunset.Unit.PartOf =
        lib.mkForce [ "wayland-session.target" ];
    })
  ]);
}
